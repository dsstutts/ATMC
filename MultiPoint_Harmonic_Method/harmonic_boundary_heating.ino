...
#define HEATER_PIN_B 4 // PWM output pin 2
//#define HEATER_PIN_C 2 // PWM output pin 3
...
Q#### -- Sinusoidal Input. Simultaneously set duty cycle, oscillation frequency, and log data, where #### represents the \
duty cycle from 0 to 1023 .\r\n\\
P### -- Oscillation period in seconds.\r\n"
};
///////// EEPROM Addresses //////
struct PID_Gains {
  double Kp;
...
{
  DC=amp*(-cos(2*3.14159/freq*ttime)+1)/2;
  iDC=(int)DC;// Cast to int
  if (iDC >= 1023)iDC = 1023;
  if (iDC <= 0)iDC = 0;
  Timer3.pwm(HEATER_PIN_A, iDC);//Set DC
...
void parseSerialInput(void) {
  ...
  if (*inbuffPtr == 'g') { //Control Setting case
    *inbuffPtr++;
    ...
    //Sinusoidal power input with data acquisition:                                                  
  if (*inbuffPtr == 'Q') { 
    saveData = true;
    pidsineUpdate = true;
    controlOn = true;
  ...
  if (*inbuffPtr == 'P') { //settemp
    *inbuffPtr++;
    while (*inbuffPtr != '\0') {
      if (((*inbuffPtr >= '0') && (*inbuffPtr <= '9')) || (*inbuffPtr == '.')) {  
        dcStr2[i] = *inbuffPtr;// Make sure they're numeric!  
        i++;
      }//End if numeric
      *inbuffPtr++;
    }// End duty cycle set while
...
{    
 ...
 if (createFile && saveData) {
  ...
  // Allows up to 9999 files: LOG0000.CSV .. LOG9999.CSV  (8.3-safe)
  // 8.3 needs up to 12 chars + NUL => 13 total
  char filename[13];

  bool opened = false;
  for (uint16_t i = 0; i < 10000; ++i) {
    // "LOG" + 4 digits = 7-char stem (<= 8), extension ".CSV"
    snprintf(filename, sizeof(filename), "LOG%04u.CSV", i);
    ...
    if (!SD.exists(filename)) {
      ...
      break; // leave the loop whether we opened or not
    }
  }

  if (!opened) {
    interrupts();
    file_err();                 // error handler
    return;
  }
  ...
  {
    amp = atoi(dcStr);
    if (amp < 0)amp = 0;// Saturate duty cycles below zero or above 1023.
  ...
  if (readTemp)
  {
    ...
    #ifdef ColdJunctionRead
      temp2[i]=ReadColdJunctionP(CSs[i]); // Read cold junction temperatures
    #elif ColdJunctionSave
